...
import { useWriteContract, useWaitForTransactionReceipt, useSimulateContract, useAccount, useReadContract, useReadContracts } from "wagmi";
...
export default function SetAsk({ ownerList, setOwnerList, bidTableList, marketplaceContract, isDisabled }) {
  ...
  useEffect(() => {
    const fetchOwnerList = async () => {
      try {
        if (!isConnected) {
          const ownerListData = await contract.methods.getOwners(allTileCoordinates).call()
  ...
  const { data: accountAskListData, isSuccess: isSuccessAccountAskListData, queryKey: accountAskListDataQueryKey } = useReadContracts({
    contracts:
      ownedTiles.map((tile) => ({
        address: marketplaceContract,
        abi: ETCeriaMarketplace_ABI,
  ...
  useEffect(() => {
    if (isSuccessAccountAskListData) {
      setAccountAskList(accountAskListData);
      if (selectedValue !== "owned Tiles") {
        setOwnedTilesWithAsks(ownedTiles.map((tile, index) => {
          const askPrice = accountAskListData.length > index ? accountAskListData[index]?.result : null;
          return { col: tile.col, row: tile.row, ask: askPrice };
        }));
      }
    }
  }, [accountAskListData, isSuccessAccountAskListData, selectedValue]);
  ...
  }, [ownedTilesWithAsks, address, selectedValue]);

  useEffect(() => {
    queryClient.invalidateQueries({ accountAskListDataQueryKey })
    queryClient.invalidateQueries({ ownerListDataQueryKey })
  }, [queryTrigger, address]);

  const { data: simulateSetAsk } = useSimulateContract({
    address: marketplaceContract,
    abi: ETCeriaMarketplace_ABI,
    functionName: 'setAsk',
    args: [selectedValue.split(',')[0], selectedValue.split(',')[1], ethers.parseEther(ask.toString()).toString()]
  });
  const { writeContract: _setAsk, data: setAskHash } = useWriteContract();

  const { isSuccess: setAskConfirmed } =
  ...
  const { data: simulateRemoveAsk } = useSimulateContract({
    address: marketplaceContract,
    abi: ETCeriaMarketplace_ABI,
    functionName: 'removeAsk',
    args: [selectedValue.split(',')[0], selectedValue.split(',')[1]],
  });
  const { writeContract: _removeAsk, data: removeAskHash } = useWriteContract();

  const { isSuccess: removeAskConfirmed } =
    useWaitForTransactionReceipt({
      hash: removeAskHash,
    })

  useEffect(() => {
    if (removeAskConfirmed) {
      setHasActiveAsk(false);
      toggleQueryTrigger();
    }
  }, [removeAskConfirmed]);

  const { data: simulateAcceptBid } = useSimulateContract({
    address: marketplaceContract,
    abi: ETCeriaMarketplace_ABI,
    functionName: 'acceptBid',
    args: [acceptBidArgs[0], acceptBidArgs[1], acceptBidArgs[2], acceptBidArgs[3]]
  });
  const { writeContract: _acceptBid, data: acceptBidHash } = useWriteContract();

  const { isSuccess: acceptBidConfirmed } =
    useWaitForTransactionReceipt({
      hash: acceptBidHash,
    })

  useEffect(() => {
    if (acceptBidConfirmed) {
      toggleQueryTrigger();
      setSelectedTile(new Set(["owned Tiles"]));
    }
  }, [acceptBidConfirmed]);
  ...
  }, [selectedValue, bidTableList, address])
  ...
  useEffect(() => {
    if (highestBid && highestBid.bidder && highestBid.offer) {
      setAcceptBidArgs([
        ...
        highestBid.bidder,
        ethers.parseEther(highestBid.offer.toString())
      ]);
    }
  ...
  return (
    <div className='flex flex-col justify-center items-center bg-inherit p-6 rounded-lg'>
      ...
      <div className="justify-center items-center">
        {highestBid && highestBid.offer > 0 && highestBid.bidder != address && ownedTiles.length > 0 && !isDisabled ? <Button variant="solid" onClick={() => _acceptBid(simulateAcceptBid?.request)} className='mb-5 mt-2 bg-green-300' >Accept {formatFloat(highestBid.offer)} ETC Bid</Button> : <Button isDisabled={true} variant="faded" className='mb-5 mt-2'>Accept Bid</Button>}
      </div>
      ...
      {hasActiveAsk ? <Button variant="faded" color="danger" onClick={() => _removeAsk(simulateRemoveAsk?.request)}>Remove Ask</Button> : <Button variant="faded" onClick={() => _setAsk(simulateSetAsk?.request)} isLoading={hasActiveAsk == null && ownerList.length > 0} isDisabled={!(ask > 0 && isConnected) || isDisabled}>Set Ask</Button>}
    </div>
  )
}