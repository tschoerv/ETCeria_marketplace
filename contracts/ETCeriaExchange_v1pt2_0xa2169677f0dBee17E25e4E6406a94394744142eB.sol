...
// credits to Cyrus Adkisson, creator of Etheria and author of the code below
...
contract ETCeriaMarketplace_v1pt2 {
    address public owner;
    address public pendingOwner;

    string public name = "ETCeriaMarketplace_v1pt2";

    ETCeria public constant etceria =
        ETCeria(address(0xB21f8684f23Dbb1008508B4DE91a0aaEDEbdB7E4));
    MapElevationRetriever public constant mapElevationRetriever =
        MapElevationRetriever(
            address(0x68549D7Dbb7A956f955Ec1263F55494f05972A6b)
        );

    uint128 public minBid = 0;
    uint256 public feeRate = 0; // in basis points (100 is 1%)
    ...
    ); // renamed some of these to conform to past tense verbs
    ...
    function setFeeRate(uint256 newFeeRate) external onlyOwner {
        ...
        require(
            (feeRate = newFeeRate) <= uint256(500),
            "EEXL: Invalid feeRate"
        ); // feeRate will revert if req fails
    }
    ...
    } // in the event of a marketplace upgrade or unforseen problem

    function _getIndex(uint8 col, uint8 row) internal pure returns (uint16) {
        require(
            _isValidColOrRow(col) && _isValidColOrRow(row),
            "EEXL: Invalid col and/or row"
        );
    ...
    function _getSurroundingWaterCount(uint8 col, uint8 row)
        internal
        view
        returns (uint8 waterTiles)
    {
        require(
            (col >= uint8(1)) && (col <= uint8(31)),
            "EEXL: Water counting requres col 1-31"
        );
        require(
            (row >= uint8(1)) && (row <= uint8(31)),
            "EEXL: Water counting requres col 1-31"
        );
        if (row % uint8(2) == uint8(1)) {
            waterTiles += _boolToUint8(
                _isWater(col + uint8(1), row + uint8(1))
            ); // northeast_hex
            waterTiles += _boolToUint8(
                _isWater(col + uint8(1), row - uint8(1))
            ); // southeast_hex
    ...
    function getOwners(uint8[][] calldata coordinates)
        public
        view
        returns (address[] memory)
    {
        address[] memory owners = new address[](coordinates.length);

        for (uint256 i = 0; i < coordinates.length; i++) {
            require(coordinates[i].length == 2, "Invalid coordinates");
            owners[i] = etceria.getOwner(coordinates[i][0], coordinates[i][1]);
        }

        return owners;
    }

    function getAsks(uint16[] calldata tileIndices)
        external
        view
        returns (uint128[] memory asks)
    {
        uint256 length = tileIndices.length;
        asks = new uint128[](length);
        for (uint256 i; i < length; ++i) {
            asks[i] = askFor[tileIndices[i]];
        }
    }
    ...
    function setAsk(
        ...
        require(
            etceria.getOwner(col, row) == msg.sender,
            "EEXL: Not tile owner"
        );
    ...
    function makeBid(
        ...
        require(msg.sender == tx.origin, "EEXL: not EOA"); // (EOA = Externally owned account) // ETCeria doesn't allow tile ownership by contracts, this check prevents black-holing
        ...
        require(
            bidOf[msg.sender].amount == uint128(0),
            "EEXL: bid exists, cancel first"
        );
    ...
    function _deleteBid(address bidder, uint64 biddersIndex) internal {
        // used by cancelBid and acceptBid
    ...
    function acceptBid(
        ...
        require(etceria.getOwner(col, row) == msg.sender, "EEXL: Not owner"); // etceria.setOwner will fail below if not owner, making this check unnecessary, but I want this here anyway
        ...
        etceria.setOwner(col, row, bidder);
        require(
            etceria.getOwner(col, row) == bidder,
            "EEXL: failed setting tile owner"
        ); // ok for require after event emission. Events are technically state changes and atomic as well.
